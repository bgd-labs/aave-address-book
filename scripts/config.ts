...
export interface Market {
  ...
  version: number;
  testnet?: boolean;
  rpcProvider: ethers.providers.FallbackProvider;
}
...
const proxyHandler = {
  ...
  apply(target, thisArg, args) {
    if (args[0] === "call") {
      return target.apply(thisArg, args).catch((e: any) => {
        // TODO:
        console.log("TODO: here we'd need automatic adding to allowlist");
        throw e;
        // try one more time
...
export const markets: Market[] = [
  ...
  provider.perform = new Proxy(provider.perform, proxyHandler);
  return {
    ...m,
    // fix checksum
